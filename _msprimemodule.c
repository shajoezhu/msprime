        ...
        PyObject *py_pop_config)
{
    ...
    double initial_size, growth_rate, multiple_merger_rate;
    int err;
    PyObject *item, *value;
    ...
    for (j = 0; j < PyList_Size(py_pop_config); j++) {
        ...
        value = get_dict_number(item, "multiple_merger_rate");
        if (value == NULL) {
            goto out;
        }
        multiple_merger_rate = PyFloat_AsDouble(value);
        err = msp_set_population_configuration(self->sim, j,
                initial_size, growth_rate, multiple_merger_rate);
        if (err != 0) {
            handle_input_error(err);
            goto out;
        }
